...
describe('Live Node Chat Server', function() {
  it('Should respond to GET requests for /log with a 200 status code', function(done) {
    request('http://127.0.0.1:3000/chatterbox', function(error, response, body) {
  ...
  it('Should 404 when asked for a nonexistent file', function(done) {
    request('http://127.0.0.1:3000/arglebargle.js', function(error, response, body) {
      expect(response.statusCode).to.equal(404);
      done();
    });
  });

  it('Should 404 when asked for a nonexistent object', function(done) {
